...
| Vegetarian marking is transcribed, not inferred | Exactly 25 items marked | The printed menu asterisks 25. Marking more asserts a dietary claim the restaurant never made — and cheesecake can contain gelatin, so an inferred flag may simply be false. |
| Verification is recorded, not assumed  | Flags may only be cleared alongside `_provenance.verifiedWithRestaurant` | Originally "≥1 item stays flagged", which was a proxy for this while everything was unverified. The user confirmed all 14 on 2026-09-02, so the proxy would now fail on *correct* data. It is replaced by the thing it stood in for: clearing a flag requires recording who confirmed what, and when. |
